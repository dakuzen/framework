  ...
  class Ngeblog {
    ...
    public function setActiveBlog($blogid)
    {
  	  ...
  	  if($this->_activeBlog == 0) {
    		...
    		throw new Exception("Could not set the active blog to $blogid, it was not found in the list of this user's blogs.");
  	  }
    }
...
<entry xmlns=\"http://www.w3.org/2005/Atom\">
  ...
  <content type=\"xhtml\">$msg</content>
      ...
      if ( $http_code=='201' ) {
        ...
        throw new Exception('Failed to add new post, Blogger returns : '.$response->getStatus());
      }
    }
...
<entry xmlns=\"http://www.w3.org/2005/Atom\">
  ...
  <content type=\"xhtml\">$msg</content>
      ...
      if ( $http_code == '200' ) {
        ...
        throw new Exception('Failed to edit post, Blogger returns : '.$response->getStatus());
      }
    }
    ...
    public function deletePost($entryid,$blogid=0)
    {
      ...
      {
        throw new Exception('Failed to delete post, Blogger returns : '.$response->getStatus());
      }
    }
    ...
    protected function _retrieveBlogInfo() 
    {
      ...
      if ($response->isSuccessful())
      {
        $resp = $this->_parseResponse($response->getBody());
        foreach ( $resp as $k=>$v ) 
        {
          $this->_blogid[$k] = $v[entryid];
          $this->_bloginfo[$k][blogid]  = $v['entryid'];
          $this->_bloginfo[$k][title]   = $v['title'];
          $this->_bloginfo[$k][url]     = $v['link'];
        }
      }
      else
      {
        throw new Exception('Failed to retrieve blog info, Blogger returns error: '.$response->getStatus());
      }
    }
    ...
    protected function _getPosts($options)
    {
      ...
      if ( ($http_code == '200') || ($http_code == '202') )
      {
        ...
        return $this->_parseResponse($xmlstr);
      }
      else
      {
	      throw new Exception('Failed to retrieve post, Blogger returned error : '.$response->getBody());
      }
    }
     ...
     * Parse response
     *
     * @param string $xmlstr response from Google
     */
    protected function _parseResponse($xmlstr)
    {
      $this->_xml_response = $xmlstr; // for debug only
      
      $resp = new SimpleXMLElement($xmlstr);
      
      if ( $resp->entry->asXML() == '' ) 
      {
        $k=0;
        $res[$k]['id'] = (string) $resp->id;
        $res[$k]['title'] = (string) $resp->title;
        $_id = explode("/",(string) $resp->id);
        $res[$k]['entryid'] = $_id[count($_id)-1];
        $_content =  (string) $resp->content->asXML();
        $res[$k]['content'] = $this->parseTextConstruct($_content,$resp->content['type']);
        $res[$k]['updated'] = (string) $resp->updated;
        $res[$k]['published'] = (string) $resp->published;
        foreach ( $resp->link as $links ) {
          if ( (string) $links['rel'] == 'alternate' )
          $res[$k]['link']=(string) $links['href'];
        }
      }
      else 
      {
        ...
        {
          $res[$k]['id'] = (string) $ent->id;
          $res[$k]['title'] = (string) $ent->title;
          $_id = explode("/",(string) $ent->id);
          $res[$k]['entryid'] = $_id[count($_id)-1];
          $_content = (string) $ent->content->asXML();
          $res[$k]['content'] = $this->parseTextConstruct($_content,$ent->content['type']);
          $res[$k]['updated'] = (string) $ent->updated;
          $res[$k]['published'] = (string) $ent->published;
          foreach ( $ent->link as $links ) {
            if ( (string) $links['rel'] == 'alternate' )
            $res[$k]['link']=(string) $links['href'];
          }
