...
function RunAdaptivePaymentSamples() {
	...
	try {
		...
		$token = CreatePay();
		GetFundingPlans($token);
		GetAvailableShippingAddress($token)	;
		...
		//Preapproval
		$preApprovalToken = Preapproval();
		
		//GetAllowedFundingSources($preApprovalToken);

		//Preapproval Details
		PreapprovalDetails($preApprovalToken);
			
		//Cancel Preapproval
		CancelPreapproval($preApprovalToken);
...
function GetFundingPlans($token){
	$getFundingPlansRequest = new GetPaymentOptionsRequest();
	$getFundingPlansRequest->payKey = $token;

	$getFundingPlansRequest->requestEnvelope = new RequestEnvelope();
	$getFundingPlansRequest->requestEnvelope->errorLanguage = "en_US";
	$ap = new AdaptivePayments();
	$response=$ap->GetFundingPlans($getFundingPlansRequest);
	if(strtoupper($ap->isSuccess) == 'FAILURE')
	{
		$FaultMsg = $ap->getLastError();
		echo "GetFundingPlans API call Failed: error Id: ";
		if(is_array($FaultMsg->error))
		{
			echo $FaultMsg->error[0]->errorId . ", error message: " . $FaultMsg->error[0]->message ;
		}
	...
	else
	{
		...
		{
			echo "GetFundingPlans API call was Successful! \n";
		}
	}
}


function GetAllowedFundingSources($token){
	$getAllowedFundingSourcesRequest = new GetAllowedFundingSourcesRequest();
	$getAllowedFundingSourcesRequest->key = $token;
	...
	$ap = new AdaptivePayments();
	$response=$ap->GetAllowedFundingSources($getAllowedFundingSourcesRequest);
	if(strtoupper($ap->isSuccess) == 'FAILURE')
	{
		$FaultMsg = $ap->getLastError();
		echo "GetAllowedFundingSource API call Failed: error Id: ";
		if(is_array($FaultMsg->error))
		{
			echo $FaultMsg->error[0]->errorId . ", error message: " . $FaultMsg->error[0]->message ;
		}
	...
	else
	{
		if($response->responseEnvelope->ack == "Success")
		{
			echo "GetAllowedFundingSource API call was Successful! \n";
		}
	}
}
function GetAvailableShippingAddress($token){
	$getAvailableShippingAddressRequest = new GetAvailableShippingAddressesRequest();
	$getAvailableShippingAddressRequest->key = $token;

	$getAvailableShippingAddressRequest->requestEnvelope = new RequestEnvelope();
	$getAvailableShippingAddressRequest->requestEnvelope->errorLanguage = "en_US";
	$ap = new AdaptivePayments();
	$response=$ap->GetAvailableShippingAddress($getAvailableShippingAddressRequest);
	if(strtoupper($ap->isSuccess) == 'FAILURE')
	{
		$FaultMsg = $ap->getLastError();
		echo "GetAvailableShippingAddress API call Failed: error Id: ";
		if(is_array($FaultMsg->error))
		{
			echo $FaultMsg->error[0]->errorId . ", error message: " . $FaultMsg->error[0]->message ;
		}
	...
	else
	{
		if($response->responseEnvelope->ack == "Success")
		{
			echo "GetAvailableShippingAddress API call was Successful! \n";
		}
	}
}
function GetShippingAddress($token){
	$getShippingAddressRequest = new GetShippingAddressesRequest();
	$getShippingAddressRequest->key = $token;

	$getShippingAddressRequest->requestEnvelope = new RequestEnvelope();
	$getShippingAddressRequest->requestEnvelope->errorLanguage = "en_US";
	$ap = new AdaptivePayments();
	$response=$ap->GetShippingAddress($getShippingAddressRequest);
	if(strtoupper($ap->isSuccess) == 'FAILURE')
	{
		$FaultMsg = $ap->getLastError();
		echo "GetShippingAddress API call Failed: error Id: ";
		if(is_array($FaultMsg->error))
		{
			echo $FaultMsg->error[0]->errorId . ", error message: " . $FaultMsg->error[0]->message ;
		}
	...
	else
	{
		if($response->responseEnvelope->ack == "Success")
		{
			echo "GetShippingAddress API call was Successful! \n";
		}
	}
}
...
function GetPaymentOption($token){
	...
	{
		$FaultMsg = $ap->getLastError();
		echo "PaymentOption API call Failed: error Id: ";
		if(is_array($FaultMsg->error))
		{
			echo $FaultMsg->error[0]->errorId . ", error message: " . $FaultMsg->error[0]->message ;
		}
	...
	else
	{
		if($response->responseEnvelope->ack == "Success")
		{
			echo "PaymentOption API call was Successful! \n";
		}
	}
}
