...
class AdaptiveAccounts extends CallerServices {     
   ...
   function AddBankAccount($ABRequest, $isRequestString = false) {
   		try {
   			if($isRequestString) {
   				return parent::callWebService($ABRequest, 'AdaptiveAccounts/AddBankAccount');
   			}
   			else {
   				return $this->callAPI($ABRequest, 'AdaptiveAccounts/AddBankAccount');	
   			}
   		}
   		catch(Exception $ex) {
 ...
 function AddPaymentCard($APCRequest, $isRequestString = false) {
   		try {
   			if($isRequestString) {
   				return parent::callWebService($APCRequest, 'AdaptiveAccounts/AddPaymentCard');
   			}
   			else {
   				return $this->callAPI($APCRequest, 'AdaptiveAccounts/AddPaymentCard');	
   			}
   		}
   		catch(Exception $ex) {
...
function GetUserAgreement ($GUARequest, $isRequestString = false) {
   		try {
   			if($isRequestString) {
   				return parent::callWebService($GUARequest, 'AdaptiveAccounts/GetUserAgreement');
   			}
   			else {
   				return $this->callAPI($GUARequest, 'AdaptiveAccounts/GetUserAgreement');	
   			}
   		}
   		catch(Exception $ex) {
