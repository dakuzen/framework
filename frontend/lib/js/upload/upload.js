...
/*flash*/ (function(f,b,d,e){var a={},g={};function c(){var h;try{h=navigator.plugins["Shockwave Flash"];h=h.description}catch(j){try{h=new ActiveXObject("ShockwaveFlash.ShockwaveFlash").GetVariable("$version")}catch(i){h="0.0"}}h=h.match(/\d+/g);return parseFloat(h[0]+"."+h[1])}d.flash={trigger:function(j,h,i){setTimeout(function(){var m=a[j],l,k;if(m){m.trigger("Flash:"+h,i)}},0)}};d.runtimes.Flash=d.addRuntime("flash",{getFeatures:function(){return{jpgresize:true,pngresize:true,maxWidth:8091,maxHeight:8091,chunks:true,progress:true,multipart:true,multi_selection:true}},init:function(m,o){var k,l,h=0,i=b.body;if(c()<10){o({success:false});return}g[m.id]=false;a[m.id]=m;k=b.getElementById(m.settings.browse_button);l=b.createElement("div");l.id=m.id+"_flash_container";d.extend(l.style,{position:"absolute",top:"0px",background:m.settings.shim_bgcolor||"transparent",zIndex:99999,width:"100%",height:"100%"});l.className="plupload flash";if(m.settings.container){i=b.getElementById(m.settings.container);if(d.getStyle(i,"position")==="static"){i.style.position="relative"}}i.appendChild(l);(function(){var p,q;p='<object id="'+m.id+'_flash" type="application/x-shockwave-flash" data="'+m.settings.flash_swf_url+'" ';if(d.ua.ie){p+='classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" '}p+='width="100%" height="100%" style="outline:0"><param name="movie" value="'+m.settings.flash_swf_url+'" /><param name="flashvars" value="id='+escape(m.id)+'" /><param name="wmode" value="transparent" /><param name="allowscriptaccess" value="always" /></object>';if(d.ua.ie){q=b.createElement("div");l.appendChild(q);q.outerHTML=p;q=null}else{l.innerHTML=p}}());function n(){return b.getElementById(m.id+"_flash")}function j(){if(h++>5000){o({success:false});return}if(g[m.id]===false){setTimeout(j,1)}}j();k=l=null;m.bind("Destroy",function(p){var q;d.removeAllEvents(b.body,p.id);delete g[p.id];delete a[p.id];q=b.getElementById(p.id+"_flash_container");if(q){i.removeChild(q)}});m.bind("Flash:Init",function(){var r={},q;try{n().setFileFilters(m.settings.filters,m.settings.multi_selection)}catch(p){o({success:false});return}if(g[m.id]){return}g[m.id]=true;m.bind("UploadFile",function(s,u){var v=s.settings,t=m.settings.resize||{};n().uploadFile(r[u.id],v.url,{name:u.target_name||u.name,mime:d.mimeTypes[u.name.replace(/^.+\.([^.]+)/,"$1").toLowerCase()]||"application/octet-stream",chunk_size:v.chunk_size,width:t.width,height:t.height,quality:t.quality,multipart:v.multipart,multipart_params:v.multipart_params||{},file_data_name:v.file_data_name,format:/\.(jpg|jpeg)$/i.test(u.name)?"jpg":"png",headers:v.headers,urlstream_upload:v.urlstream_upload})});m.bind("CancelUpload",function(){n().cancelUpload()});m.bind("Flash:UploadProcess",function(t,s){var u=t.getFile(r[s.id]);if(u.status!=d.FAILED){u.loaded=s.loaded;u.size=s.size;t.trigger("UploadProgress",u)}});m.bind("Flash:UploadChunkComplete",function(s,u){var v,t=s.getFile(r[u.id]);v={chunk:u.chunk,chunks:u.chunks,response:u.text};s.trigger("ChunkUploaded",t,v);if(t.status!==d.FAILED&&s.state!==d.STOPPED){n().uploadNextChunk()}if(u.chunk==u.chunks-1){t.status=d.DONE;s.trigger("FileUploaded",t,{response:u.text})}});m.bind("Flash:SelectFiles",function(s,v){var u,t,w=[],x;for(t=0;t<v.length;t++){u=v[t];x=d.guid();r[x]=u.id;r[u.id]=x;w.push(new d.File(x,u.name,u.size))}if(w.length){m.trigger("FilesAdded",w)}});m.bind("Flash:SecurityError",function(s,t){m.trigger("Error",{code:d.SECURITY_ERROR,message:d.translate("Security error."),details:t.message,file:m.getFile(r[t.id])})});m.bind("Flash:GenericError",function(s,t){m.trigger("Error",{code:d.GENERIC_ERROR,message:d.translate("Generic error."),details:t.message,file:m.getFile(r[t.id])})});m.bind("Flash:IOError",function(s,t){m.trigger("Error",{code:d.IO_ERROR,message:d.translate("IO error."),details:t.message,file:m.getFile(r[t.id])})});m.bind("Flash:ImageError",function(s,t){m.trigger("Error",{code:parseInt(t.code,10),message:d.translate("Image error."),file:m.getFile(r[t.id])})});m.bind("Flash:StageEvent:rollOver",function(s){var t,u;t=b.getElementById(m.settings.browse_button);u=s.settings.browse_button_hover;if(t&&u){d.addClass(t,u)}});m.bind("Flash:StageEvent:rollOut",function(s){var t,u;t=b.getElementById(m.settings.browse_button);u=s.settings.browse_button_hover;if(t&&u){d.removeClass(t,u)}});m.bind("Flash:StageEvent:mouseDown",function(s){var t,u;t=b.getElementById(m.settings.browse_button);u=s.settings.browse_button_active;if(t&&u){d.addClass(t,u);d.addEvent(b.body,"mouseup",function(){d.removeClass(t,u)},s.id)}});m.bind("Flash:StageEvent:mouseUp",function(s){var t,u;t=b.getElementById(m.settings.browse_button);u=s.settings.browse_button_active;if(t&&u){d.removeClass(t,u)}});m.bind("Flash:ExifData",function(s,t){m.trigger("ExifData",m.getFile(r[t.id]),t.data)});m.bind("Flash:GpsData",function(s,t){m.trigger("GpsData",m.getFile(r[t.id]),t.data)});m.bind("QueueChanged",function(s){m.refresh()});m.bind("FilesRemoved",function(s,u){var t;for(t=0;t<u.length;t++){n().removeFile(r[u[t].id])}});m.bind("StateChanged",function(s){m.refresh()});m.bind("Refresh",function(s){var t,u,v;n().setFileFilters(m.settings.filters,m.settings.multi_selection);t=b.getElementById(s.settings.browse_button);if(t){u=d.getPos(t,b.getElementById(s.settings.container));v=d.getSize(t);d.extend(b.getElementById(s.id+"_flash_container").style,{top:u.y+"px",left:u.x+"px",width:v.w+"px",height:v.h+"px"})}});m.bind("DisableBrowse",function(s,t){n().disableBrowse(t)});o({success:true})})}})})(window,document,plupload);
/*create*/ jQuery.extend({Class:function(){},Interface:function(){}});jQuery.extend(jQuery.Class,{extend:function(){var a,b,c,d,e,f,g=arguments[0],h=arguments[1]||{},i=2,j=arguments.length,k=false;if(typeof h==="boolean"){k=h;h=arguments[2]||{};i=3}if(typeof h!=="object"&&!jQuery.isFunction(h)){h={}}if(j===i){h=this;--i}for(;i<j;i++){if((a=arguments[i])!=null){for(b in a){if(b!=g){c=h[b];d=a[b];if(h===d){continue}if(k&&d&&(jQuery.isPlainObject(d)||(e=jQuery.isArray(d)))){if(e){e=false;f=c&&jQuery.isArray(c)?c:[]}else{f=c&&jQuery.isPlainObject(c)?c:{}}h[b]=jQuery.extend(k,f,d)}else if(d!==undefined){h[b]=d}}}}}return h},getFunctionBody:function(a){var b=a.toString().match(/\{([\s\S]*)\}/m)[1];return $.trim(b.replace(/^\s*\/\/.*$/mg,""))},Function:function(){}});jQuery.extend(jQuery.Class.Function,{Empty:function(){}});jQuery.extend(jQuery.Interface.prototype,{attributes:[],properties:[],methods:[]});jQuery.extend(jQuery.Class,{createGetSet:function(object,name,element){eval('object.prototype["get'+name+'"] = function () { return this.'+element+"; };");eval('object.prototype["set'+name+'"] = function () { this.'+element+" = arguments[0]; };");if(!object.prototype[element]){object.prototype[element]=""}},createGetSetRuntime:function(object,name,element){eval('object.constructor.prototype["get'+name+'"] = function () { return this.'+element+"; };");eval('object.constructor.prototype["set'+name+'"] = function () { this.'+element+" = arguments[0]; };");if(!object.constructor.prototype[element]){object.constructor.prototype[element]=""}},createNamespace:function(a){var b=a.split("."),c="window";while(b.length){var a=b.shift();c+='["'+a+'"]'}return c},isIn:function(a,b){for(var c in a.prototype){if(c==b){if(jQuery.isArray(a.prototype[c]))return 0;else if(jQuery.isFunction(a.prototype[c]))return 1;else if(jQuery.isEmptyObject(a.prototype[c])||jQuery.isPlainObject(a.prototype[c]))return 2;else if(jQuery.isXMLDoc(a.prototype[c]))return 3;else return 0}}return-1},create:function(){function handle(){if(this.options.abstract){throw new Error("abstract classes cannot be instantiated")}try{this.initialize.apply(this,arguments)}catch(a){}}var parent=null,elements=null,base=null,options={"abstract":false,getset:[],"implements":[],module:""};if(jQuery.isFunction(arguments[0])){parent=arguments[0];elements=arguments[1];if(arguments[2]){jQuery.extend(options,arguments[2]||{})}}else{elements=arguments[0];if(arguments[1]){jQuery.extend(options,arguments[1]||{})}}jQuery.extend(handle.prototype,jQuery.Class);if(parent){function clone(a){if(a==null||typeof a!="object")return a;var b=a.constructor();if(b==undefined)b={};for(var c in a)b[c]=clone(a[c]);return b}for(property in parent.prototype){if(property!="superClass"){switch($.type(parent.prototype[property])){case"function":eval("handle.prototype[property] = "+parent.prototype[property]);break;case"object":handle.prototype[property]=clone(parent.prototype[property]);break;default:handle.prototype[property]=parent.prototype[property];break}}}handle.prototype.superClass=parent.prototype}jQuery.extend(handle.prototype,elements||{});handle.prototype.options=options;if(options.getset.length>0){for(var i=0;i<options.getset.length;i++){var name=options.getset[i][0],element=options.getset[i][1];this.createGetSet(handle,name,element)}}if(options.implements.length>0){var attributesMustImplemented=[],propertiesMustImplemented=[],methodsMustImplemented=[];for(var i=0;i<options.implements.length;i++){jQuery.merge(attributesMustImplemented,options.implements[i].attributes);jQuery.merge(propertiesMustImplemented,options.implements[i].properties);jQuery.merge(methodsMustImplemented,options.implements[i].methods)}var didNotImplemented=false,msg="must be implemented";for(var i=0;i<attributesMustImplemented.length;i++){var result=this.isIn(handle,attributesMustImplemented[i]);if(result!=0&&result!=2){didNotImplemented=true;msg="attribute: "+attributesMustImplemented[i]+", "+msg}}for(var i=0;i<propertiesMustImplemented.length;i++){var resultGet=this.isIn(handle,"get"+propertiesMustImplemented[i]),resultSet=this.isIn(handle,"set"+propertiesMustImplemented[i]);if(resultGet!=1){didNotImplemented=true;msg="property: get"+propertiesMustImplemented[i]+", "+msg}else if(resultSet!=1){didNotImplemented=true;msg="property: set"+propertiesMustImplemented[i]+", "+msg}}for(var i=0;i<methodsMustImplemented.length;i++){var result=this.isIn(handle,methodsMustImplemented[i]);if(result!=1){didNotImplemented=true;msg="method: "+methodsMustImplemented[i]+", "+msg}}if(didNotImplemented){throw new Error(msg)}}if(options.module!=""){var names=options.module.split("."),name=names[0];if(window[name]==null||window[name]==undefined)window[name]=new function(){};for(var i=1;i<names.length;i++){name+="."+names[i];eval("if ("+this.createNamespace(name)+" == null || "+this.createNamespace(name)+" == undefined) "+this.createNamespace(name)+" = new function() {};")}eval("jQuery.extend("+this.createNamespace(name)+", handle.prototype);")}return handle},addMethods:function(){if(arguments[0]){jQuery.extend(this.constructor.prototype,arguments[0])}},addAttributes:function(){if(arguments[0]){jQuery.extend(this.constructor.prototype,arguments[0])}},addProperty:function(){try{var a=arguments[0],b=arguments[1];this.createGetSetRuntime(this,a,b)}catch(c){}},property:function(){if(arguments.length==1&&this.constructor.prototype.hasOwnProperty("get"+arguments[0])){if(arguments[0]){if(jQuery.isFunction(this["get"+arguments[0]])){return this["get"+arguments[0]]()}}}else if(this.constructor.prototype.hasOwnProperty("set"+arguments[0])){if(jQuery.isFunction(this["set"+arguments[0]])){this["set"+arguments[0]](arguments[1]);return this["get"+arguments[0]]()}}},equal:function(){if(arguments[1]){return arguments[0].constructor.prototype==arguments[1].constructor.prototype}else{return this.constructor.prototype==arguments[0].constructor.prototype}},clone:function(){function a(){try{this.initialize.apply(this,arguments)}catch(a){}}if(arguments[0]==true){jQuery.extend(a.prototype,this.constructor.prototype);return a}else{jQuery.extend(a.constructor.prototype,this.constructor.prototype);return a}},toString:function(){return"Design By Hassan Jodat Shandi"},base:function(){try{var methodName=arguments[0];this["temp"+methodName]=this.superClass[methodName];var temp=this.superClass;if(this.superClass.superClass){this.superClass=this.superClass.superClass}var caller='this["temp" + methodName](';if(arguments[1])caller+="arguments[1]";for(var i=2;i<arguments.length;i++)caller+=", arguments["+i+"]";caller+=");";eval(caller);this["temp"+methodName]=null;this.superClass=temp}catch(ex){}}})

var UploadUtil = $.Class.create(plupload.Uploader,{ 

	initialize: function(settings) {
	
		settings 			= settings==undefined ? {} : settings;
		settings.runtimes 		= settings.runtimes!=undefined ? settings.runtimes : "html5,flash,silverlight,html4";
		settings.max_uploads 		= settings.max_uploads!=undefined ? settings.max_uploads : '0';
		//settings.max_file_size 		= settings.max_file_size!=undefined ? settings.max_file_size : '10mb';
		settings.flash_swf_url 		= settings.flash_swf_url!=undefined ? settings.flash_swf_url : '/lib/js/plupload/plupload.flash.swf';
		settings.silverlight_xap_url 	= settings.silverlight_xap_url!=undefined ? settings.silverlight_xap_url : '/lib/js/plupload/plupload.silverlight.xap';
		...
		settings.multi_selection 	= settings.max_uploads != 1;
		settings.file_list	 	= settings.file_list ? settings.file_list : null;
		settings.multipart_params 	= settings.multipart_params ? settings.multipart_params : settings.data;

		this.base("constructor",settings);
		...
		this.bind("FilesAdded", function(up, files) {
			...
			if(up.get_max_uploads()==1) {

				up.refresh();

				$.each(up.files, function(i1, file1) {
		...
		if(settings.file_list)
			this.bind_default_filelist(settings.file_list);
	},
	...
	bind_default_filelist: function(filelist) {
		...
		this.bind("Init", function(up, params) {
			...
			if(params.runtime="")
				filelist.html("No runtime found");
		});					
		...
		this.bind("FilesAdded", function(up, files) {
			...
			$.each(files, function(i, file) {
				...
				s = plupload.formatSize(file.size).toString();

				fl = $('<div id="file-list-' + file.id + '" class="file-list-item"></div>');

				fl.append(file.name);
				fl.append(" (" + s + ")");
				fl.append('<span class="file-list-status"></span>');
	...
	set_max_uploads: function(max_uploads) {				
		this.settings.multi_selection = max_uploads!=1;
		this.settings.max_uploads = max_uploads;
	},

	get_max_uploads: function() {
		return this.settings.max_uploads;
	},

	enable_single_selection: function() {
		this.set_max_uploads(1);
	},

	has_files: function() {
		return this.total.uploaded != this.files.length;
	},
...
(function($){ $.fn.upload = function(options,arg1,arg2) {

	if(options=="queued") {
		
		queued = 0;
		$(this).each(function(i,el) {
			uu = $(el).get(0).upload_util;
			queued += uu.files.length - uu.total.uploaded;
		});		
	...
	if(options=="finished") {
		$.fn.upload.finished = arg1;
		return true;
	}

	options = options ? options : {};
	
	$.fn.upload.instances = $(this);	
	...
	$(this).each(function(i,el) {
		...
		if(typeof options == "string") {
			...
			if(options=="url") {
				uu.settings.url = arg1;
				
				if(arg2)
					uu.settings.multipart_params = arg2;
			}

			if(options=="data")
				uu.settings.multipart_params = arg1;
			...
			if(options=="start") {				
				if(uu.files.length != (uu.files.uploaded ? uu.files.uploaded : 0))
					uu.start();
			}		

			if(options=="uploaded") {
				
				uu.unbind("FileUploaded");
				
				uu.bind("FileUploaded", function(up,file,response) {
					response = $.parseJSON(response.response);
					arg1(up,file,response);
				});		
			}

			if(options=="added") {

				uu.unbind("FilesAdded");
				
				uu.bind("FilesAdded", function(up,file) {
					arg1(up,file);
				});		
			}
		...
		if(!$(el).attr("id"))
			$(el).attr("id","upload-container-" + Math.ceil(Math.random()*100000));

		if(!$(el).children().length) 
			$(el).html($("<div>").text($(el).text()));
		
		opt.container = $(el).attr("id");

		if(!opt.browse_button) {
			
			ub = $(el).find(".upload-browse");
			...
			if(ub.length) {
				...
				if(!ub.attr("id"))
					ub.attr("id","upload-browse-" + Math.ceil(Math.random()*100000));
				
				opt.browse_button = ub.attr("id");
			}
		}

		uu = new UploadUtil(opt);
		uu.init();		
		
		if(opt.finished) 
			$.fn.upload.finished = opt.finished;
			
		uu.bind("FileUploaded", function(up,file,response) {
			
			response = $.parseJSON(response.response);

			finished = $.fn.upload.finished;

			if($.fn.upload.instances.upload("queued")==0 && finished)
				finished(up,file,response);
		});

		if(opt.progress)
			uu.bind("UploadProgress", opt.progress);

		if(opt.error) 
			uu.bind("Error", opt.error);

		if(opt.added) 
			uu.bind("FilesAdded", opt.added);
		...
		if(opt.uploaded) 
			...
			uu.bind("FileUploaded",function(up,file,response) {
				...
				up.settings.uploaded(up,file,response);
			});

		uu.element = $(el);
		$(el).get(0).upload_util = uu;
	});
}})(jQuery);
