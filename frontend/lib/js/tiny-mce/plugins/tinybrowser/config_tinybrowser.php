<?php
	...
	//$this->_root_directory 		= FILE_UTIL::get_sanitized_directory();
	...
	*/
	...
	$tinybrowser['sessioncheck'] = 'htmleditor'; //name of session variable to check

	// Random string used to secure Flash upload if session control not enabled - be sure to change!
	$tinybrowser['obfuscate'] = '1kjhg3iu132h42345ds5';
	...
	$tinybrowser['docroot'] = $html_editor_cmodel->get_root_directory();
	
	// Folder permissions for Unix servers only
	$tinybrowser['unixpermissions'] = 0777;

	// File upload paths (set to absolute by default)
	$tinybrowser['path']['image'] = ""; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
	$tinybrowser['path']['media'] = ""; // Media files location
	...
	$tinybrowser['link']['image'] = $html_editor_cmodel->get_root_url(); // Image links
	$tinybrowser['link']['media'] = $html_editor_cmodel->get_root_url(); // Media links
	$tinybrowser['link']['file']  = $html_editor_cmodel->get_root_url(); // Other file links
