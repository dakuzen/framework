...
function get_insensative_extension($extension) {
 	//Raymond Gigliotti
	 //removed  *.
	return "{".strtoupper($extension).",".strtolower($extension)."}";
}
...
if($level==0 && is_dir($root.$tree.$branch))
	...
	foreach($filetypes as $filetype)
	 ...
	 //Raymond Gigliotti
	 //added  + GLOB_BRACE
   	$filenum = $filenum + count(glob($root.$tree.$branch.get_insensative_extension($filetype),GLOB_NOSORT + GLOB_BRACE));
   	}
		...
		foreach($filetypes as $filetype)
		   {
		   	 //Raymond Gigliotti
	 		//added  + GLOB_BRACE
			$filenum = $filenum + count(glob($root.$tree.$branch.$dirname.'/'.get_insensative_extension($filetype),GLOB_NOSORT + GLOB_BRACE));
			}
		$indent = '';
