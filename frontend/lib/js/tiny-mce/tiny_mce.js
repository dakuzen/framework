		...
		processHTML : function(h) {
			...
			if (s.keep_values) {
				...
				if (/<script|noscript|style/i.test(h)) {
					...
					h = h.replace(/<script([^>]+|)>([\s\S]*?)<\/script>/gi, function(v, attribs, text) {
						...
						attribs = attribs.replace(/src=\"([^\"]+)\"?/i, function(a, url) {
							...
							/*
							 *  Raymond Gigliotti
							 *  removed the '_mce_' prefix  original code: return '_mce_src="' + url + '"';
							 */
						...
						/*
						 *  Raymond Gigliotti
						 *  removed the 'mce' prefix  original code: return '<mce:script' + attribs + '>' + text + '</mce:script>';
						 */
					...
					h = h.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/gi, function(v, attribs, text) {
						...
						/*
						 *  Raymond Gigliotti
						 *  removed the 'mce' prefix  original code: return '<mce:style' + attribs + '>' + text + '</mce:style><style ' + attribs + ' _mce_bogus="1">' + text + '</style>';
						 */
						 
						return '<style' + attribs + '>' + text + '</style>';
					});
				...
				function processTags(html) {
					...
					/*
					 *  Raymond Gigliotti
					 *  skipped the process tag html function
					 */ 
