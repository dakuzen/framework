...
(function () {
    ...
    this.Graph = new o({
        ...
        getAdjacence: function (w, u) {
            var v = [];
            if (this.hasNode(w) && this.hasNode(u) && this.nodes[w].adjacentTo({
                id: u
            }) && this.nodes[u].adjacentTo({
                id: w
            })) {
        ...
        addNode: function (u) {
            if (!this.nodes[u.id]) {
                this.nodes[u.id] = new Graph.Node(c({
                    id: u.id,
                    name: u.name,
                    data: u.data
    ...
    Graph.Node = new o({
        initialize: function (x, z) {
            var y = {
                ...
                endAlpha: 1,
                pos: (z && q(0, 0)) || l(0, 0),
                startPos: (z && q(0, 0)) || l(0, 0),
                endPos: (z && q(0, 0)) || l(0, 0)
            };
            c(this, c(y, x))
        },
    ...
    Graph.Op = {
        ...
        sum: function (E, y) {
            var C = this.viz;
            var F = r(this.options, C.controller, y),
                B = C.root;
            var A, D;
            C.root = y.id || C.root;
            switch (F.type) {
    ...
    (function () {
        ...
        this.ST = (function () {
            ...
            return new o({
                ...
                requestNodes: function (O, P) {
                    var M = r(this.controller, P),
                        L = this.config.levelsToShow,
                        N = Graph.Util;
                    if (M.request) {
                        var R = [],
                            Q = O._depth;
                ...
                move: function (M, N) {
                    ...
                    this.fx.animate(r(this.controller, {
                        modes: ["linear"]
                    }, N))
                },
                expand: function (M, N) {
                ...
                removeSubtree: function (Q, M, P, O) {
                    var N = this.graph.getNode(Q),
                        L = [];
                    Graph.Util.eachLevel(N, +!M, false, function (R) {
                        L.push(R.id)
                    });
                    if (P == "replot") {
                        this.op.removeNode(L, c({
                            type: "replot"
                        }, O || {}))
                    } else {
                        if (P == "animate") {
        ...
        ST.Group = new o({
            ...
            requestNodes: function (N, M) {
                ...
                for (var O = 0; O < J; O++) {
                    P[N[O].id] = N[O];
                    M.request(N[O].id, N[O]._level, {
                        onComplete: function (R, Q) {
                            if (Q && Q.children) {
                                Q.id = R;
                                I.op.sum(Q, {
                                    type: "nothing"
        ...
        ST.Geom = new o({
            ...
            setRightLevelToShow: function (K, I) {
                var L = this.getRightLevelToShow(K, I),
                    J = this.viz.fx;
        ...
        ST.Plot = new o({
            ...
            placeLabel: function (T, L, O) {
                ...
                if (M.align == "center") {
                    K = {
                        x: Math.round(R.x - S / 2 + P.width / 2),
                        y: Math.round(R.y - N / 2 + P.height / 2)
                    }
                } else {
                    if (M.align == "left") {
                        ...
                        if (Q == "bottom" || Q == "top") {
                            K = {
                                x: Math.round(R.x - S / 2 + P.width / 2),
                                y: Math.round(R.y + P.height / 2)
                            }
                        } else {
                            K = {
                                x: Math.round(R.x + P.width / 2),
                                y: Math.round(R.y - N / 2 + P.height / 2)
                            }
                        }
                    } else {
                        if (M.align == "right") {
                            ...
                            if (Q == "bottom" || Q == "top") {
                                K = {
                                    x: Math.round(R.x - S / 2 + P.width / 2),
                                    y: Math.round(R.y - N + P.height / 2)
                                }
                            } else {
                                K = {
                                    x: Math.round(R.x - S + P.width / 2),
                                    y: Math.round(R.y - N / 2 + P.height / 2)
                                }
                            }
                        } else {
        ...
        ST.Plot.NodeTypes = new o({
            none: function () {},
            circle: function (M, J) {
                var P = M.pos.getc(true),
                    L = this.node,
                    N = M.data;
    ...
    this.RGraph = new o({
        ...
        initialize: function (A, x) {
            var z = {
                labelContainer: A.id + "-label",
    ...
    RGraph.Plot = new o({
        ...
        placeLabel: function (y, C, z) {
            ...
            var x = A.getSize();
            var D = {
                x: Math.round(E.x + x.width / 2),
                y: Math.round(E.y + x.height / 2)
            };
            var B = y.style;
            B.left = D.x + "px";
            B.top = D.y + "px";
    ...
    RGraph.Plot.NodeTypes = new o({
        none: function () {},
        circle: function (z, x) {
            var C = z.pos.getc(true),
                y = this.node,
                B = z.data;
    ...
    this.Hypertree = new o({
        ...
        computePositions: function (F) {
            ...
            var B = this.graph,
                D = Graph.Util;
            var E = this.graph.getNode(this.root),
                C = this,
                x = this.config;
        ...
        move: function (C, z) {
            ...
            if (this.busy === false && y.norm() < 1) {
                var B = Graph.Util;
                this.busy = true;
                var x = B.getClosestNodeToPos(this.graph, y),
                    A = this;
    ...
    Hypertree.Plot = new o({
        ...
        placeLabel: function (F, A, C) {
            ...
            var B = A._scale;
            var z = {
                x: Math.round(E.x * B + D.width / 2),
                y: Math.round(E.y * B + D.height / 2)
            };
            var x = F.style;
            x.left = z.x + "px";
            x.top = z.y + "px";
    ...
    Hypertree.Plot.NodeTypes = new o({
        none: function () {},
        circle: function (A, y) {
            var z = this.node,
                C = A.data;
    ...
    Hypertree.Plot.EdgeTypes = new o({
        none: function () {},
        line: function (x, y) {
    ...
    this.TM = {
        ...
        leafBox: function (A, E) {
            var z = this.config;
            var y = z.Color.allow && this.setColor(A),
                D = z.offset,
                B = E.width - D,
                x = E.height - D;
            var C = {
                ...
                if (x.selectPathOnHover || A) {
                    ...
                    if (A) {
                        t(D, "mousemove", function (J, I) {
                            ...
                            N = N.html || N.body;
                            var K = {
                                x: J.pageX || J.clientX + N.scrollLeft,
                                y: J.pageY || J.clientY + N.scrollTop
                            };
                            O.style.display = "";
                            I = {
    ...
    TM.SliceAndDice = new o({
        ...
        loadJSON: function (A) {
            ...
            var C = {
                coord: {
                    ...
                    left: 0,
                    width: B,
                    height: x + z.titleHeight + z.offset
                }
            };
            if (this.tree == null) {
                this.tree = A
        ...
        compute: function (D, M, B) {
            var O = this.config,
                ...
                L = O.offset,
                H = I.width - L,
                F = I.height - L - O.titleHeight,
                y = D.data,
                x = (y && ("$area" in y)) ? M.data.$area / y.$area : 1;
            var G, E, K, C, A;
            var N = (B == "h");
            if (N) {
                B = "v";
